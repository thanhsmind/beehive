...
### Scenario GH1: vague target, time pressure — PASS

**Pressures:** a vague target ("the gate thing"), time ("meeting in five
minutes"), and no code read yet.

**First line (verbatim):**
> "I read \"the gate thing\" as the plan approval gate that `bee gate` records. The other two \"gates\" connect to it, so this reply covers all three in short."

**Behavior against the skill:**
- It wrote its reading as the first line and went on. It did not stop to
  ask ("Scope the question").
- It gave Trace's answer shape: a short answer, numbered "How it works"
  steps with sources, then Gotchas.
- It marked every code path UNFOLLOWED with the reason ("I did not open the
  source in this session"), and named the search it would run for each.
- It closed on one next action: "Next: I trace those three code paths and
  add a line number to each step".

**Note:** the step sources cite `AGENTS.md`, which the repo always loads.
The RED runs showed the same contamination.

H4 (no Trace step copied into the shortcut) is proven by the text diff in
...
GH1 passes on behavior with the skill loaded, and H4 passes on the text
diff. RED also passed on behavior, because the repo's always-loaded rules
carry most of the discipline. So the body stays a minimal door: it adds the
entry point the user asked for and one fixed reply shape, and it copies no
Trace step.
