...
**A named smell vocabulary.** A documented repo standard always wins —
where it endorses something a smell below would flag, suppress the
smell. Every smell is a labelled heuristic ("possible Feature Envy"), a
judgement call, never a hard violation:

- **Mysterious Name** — a function, variable, or type whose name
  doesn't reveal what it does or holds -> rename it; if no honest name
  comes, the design's murky.
- **Duplicated Code** — the same logic shape appears in more than one
  place -> extract the shared shape, call it from both.
...
- **Repeated Switches** — the same `switch`/`if`-cascade on the same
  type recurs across the code -> replace with polymorphism, or one map
  every site shares.
...
- **Speculative Generality** — abstraction, parameters, or hooks added
  for needs nothing yet has -> delete it; inline back until a real need
  shows.
