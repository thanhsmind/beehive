...
- Finish with: node .bee/bin/bee.mjs cells finish --id {{cell_id}} --outcome "<one line>" --files <a,b> — it runs the project's declared commands.test first: green caps the cell, red refuses the cap and quotes the failing excerpt. The red is the work: fix it and re-run finish; never build on a red base.
