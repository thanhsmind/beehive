...
- Seeded host config: six hooks all on, `gate_bypass: false`, `team.claude` = code sonnet / read haiku / extraction haiku / generation sonnet, `team.codex` all null (`onboard/templates.rs:194-222`).
...
- Route vocabularies: class `feature|bugfix|docs|refactor|research|release|spike|perf|content`; lane `docs|tiny|small|spike|standard|high-risk`. Lane transitions: promotion always allowed, high-risk never demotes, hard-gate flags block demotion, one demotion per feature ever.
