...
- **Streams.** `state session list`, `work show`, and `work set` print their human text on **stdout**, verified live in this repository. [invocation](../foundations/invocation.md) records the contract as "human messages on stderr". One of the two is wrong; the divergence is recorded here rather than resolved, since invocation owns the contract.
