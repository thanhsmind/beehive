...
- The statusline script is vendored by default: when `.claude/settings.json` carries no `statusLine` key, onboarding writes the project-level entry and vendors `.claude/statusline-command.sh`; an existing key is preference and left alone, and `--no-statusline` or `.bee/config.json` `statusline:false` suppresses both.
