...
The route record validates on write: `class` from `feature|bugfix|docs|refactor|research|release|spike|perf|content`; `lane` from `docs|tiny|small|spike|standard|high-risk`; flags from a fixed ten-value list. The record carries a rationale and timestamps.
