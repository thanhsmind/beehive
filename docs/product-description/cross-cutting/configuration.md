...
| `team.claude.<role>`, `team.codex.<role>` | seeded: claude `code`=sonnet, `read`=haiku, `extraction`=haiku, `generation`=sonnet; codex all `null` | Which model each dispatched role runs on, and therefore what `bee dispatch prepare` returns and what the model guard repairs a mismatched dispatch to. Slot shapes: a plain string, `{model, effort}`, `{kind:"cli", …}`, `{kind:"herding", …}`, or `null`. [workers](../delegation/workers.md) owns roles and repair. Under `bee-plan/v2`, role assignments are planned and enforced at dispatch and native guard doors. | an unconfigured role name falls through and warns during ad hoc dispatches; under v2 plans, unconfigured or mismatched roles refuse |
