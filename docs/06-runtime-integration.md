...
| Gate 2 "no execution before its approval" (folds in the old standalone execution gate) | `bee-write-guard` blocks source writes pre-approval | `bee.mjs cells claim` refuses while `approved_gates.execution: false`; workers only act on claimed cells; AGENTS.md red-flag rule |
