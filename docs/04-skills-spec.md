...
The standalone validating skill is deleted (validation-diet D1) — no standalone reality-gate skill remains. Its reality gate and cold-pickup cell review folded into `bee-planning` above (D1/D5); the feasibility matrix, the plausibility ban's home, and the adversarial plan-checker's persona panel moved with the review wave; the decision vocabulary and the standalone Gate 3 it used to earn are retired, folded into Gate 2 (D2).
