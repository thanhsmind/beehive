...
The extension also runs an advisory result inbox drain on `session_start` to poll for detached `bee herding run` background job results under `.bee/result-inbox/<token>/` and inject them into the session via `pi.sendUserMessage`. The drain (`renderResultInjection`) extracts only result header fields (`job_id`, `cell_id`, `status`, `summary`, `proof`, `report_path`) from `result-N.json`, safely ignoring the run verb's outer envelope keys (`retryable`, `git`).
