...
- **A name nothing configures WARNS on stderr**, naming what it fell through to — e.g. `bee: model role "tset" is not configured in models.claude of .bee/config.json — falling through to "code"`. It is never silently accepted, and it never hands back another role's model as if the config had named it. (bee's own tail names `code` and `read` are exempt from that warning: they are absent from every host that onboarded before roles shipped, and a warning that fires on every dispatch is one nobody reads. An operator-invented name like `test` or `desgin` still warns loudly.)
