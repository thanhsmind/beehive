...
| Model selection declared per dispatch (adopted as cost tiers, re-cut as job roles) | compound-engineering | Required cell `role` + `bee cells escalate`; spawn guidance in `bee-swarming` / `bee-planning` (review wave) / `bee-reviewing` |
...
- **Trace depth by lane:** tiny work records a one-line summary + outcome; high-risk work records actions, files, decisions, errors, friction. Mechanical field checks, agent can self-score.
...
- **Model selection declared per spawn — taken as cost classes, kept as job roles.** CE declares a cost tier per dispatch; bee keeps the declarable-per-spawn part and re-cuts the axis, because nobody can judge which model is "good at generation" while real model strengths are job-shaped. A cell carries a REQUIRED `role` naming the job (`code`, `read`, `test`, `docs`, `design` as guidance only — any non-empty name is legal, since validation checks presence and shape and never membership), and `models.<runtime>` maps that role to a model. A dispatch asks for an ordered list headed by the cell's role and ending in the historical `extraction`/`generation` names, so an unconfigured role falls through with a warning rather than failing, and no existing host changes model. `ceiling` stopped being a tier value and became an explicit escalation flag — `bee cells escalate --id <id> [--reason <text>] [--off]`, run on the session model and charged against a 40% ration. On runtimes without per-agent model selection, a role degrades to the inherited model and cost control falls back to read budgets and output caps. This formalizes superpowers' "turn count beats token price" into something declarable per spawn.
...
3. **Three-runtime as a contract, not a port — one brain, three belts (opencode-support D1).** The workflow artifacts (`.bee/`, `docs/history/`, cells, gates) are runtime-neutral, and the rules are enforced from the one shared `bee` binary in three ways: the CLI helpers enforce mechanically on *every* runtime (cap-requires-verify, gate-locked claiming, reservation conflicts); the 9-hook skeleton — rendered from one shared catalog and wired for Claude Code and Codex — adds harness-level guards, deduped reminders, and chain-nudges on top; and OpenCode carries its own hand-written third belt (`.opencode/plugins/bee-guard.ts`, D2) calling the same helper entry points, because its block path (a thrown error) cannot be a rendered JSON manifest sharing the other two's catalog. Codex and OpenCode are not degraded copies; both run the same helper-enforced rules, Codex with AGENTS.md as its bootstrap vector. Full design and parity matrix: [06-runtime-integration.md](06-runtime-integration.md).
