...
| D9 | Stale numbered-rule pointers are repaired at the source of truth. The known set is enumerated with evidence in `reports/stale-rule-pointers.md`; that inventory is a floor, not a total — the implementing cell re-runs discovery and reports the final count. **AMENDED 2026-07-29 (decision `3a756908`), see below.** | Same root cause: renumbering done under size pressure. The inventory is explicitly unproven-exhaustive because one of its 13 rows was found only while verifying the others — trusting the count would repeat the original error. |

**D9 amendment — delivered scope is narrower than locked.** As locked, D9 read "repaired repo-wide **in this feature**". Delivered: the 12 pointers that were direct collateral of the byte-diet renumbering are repaired (cells `bfr-2`, `bfr-6`); the 9 further defects the fresh sweep surfaced are deferred to the backlog as P3, three of them likely predating the byte diet entirely. The user narrowed scope mid-flight after asking whether the work had drifted from removing the byte-ceiling law, and their call was right — the sweep had grown past the law that caused it. Recorded as decision `3a756908` per the governance clause above, which this feature initially failed to honour: the cut lived only in a dropped cell's reason and a backlog row until compounding caught it. D9 also mixed altitudes by locking "as their own cell", which is cell shape, not a product outcome.
