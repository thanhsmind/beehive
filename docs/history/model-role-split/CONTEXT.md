...
- [ ] **Does `--kind extract` (D12) still ship as its own step?** D2's
...
- [ ] **How the 506-cell backfill (D9) is applied** — a migration verb,
      a one-time script, or lazily on read. All cells are capped.
- [ ] **The dispatch's ordered list per consumer** — the exact list each
