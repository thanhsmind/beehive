...
| D2 | The role plan classifies every workflow stage as `required`, `conditional`, or `not-applicable`. Applicable stages name one configured role and explain the choice. | Reviewers must see omissions as explicit decisions, not missing rows. |
...
| D6 | Release execution requires a current dispatch authorization for the approved `deployment` stage and its `deploy` role. `scripts/release.sh` remains the release implementation. | Release `2.37.2` proved that a direct script call can bypass a configured deploy role. Decision `c0a4d406`. |
