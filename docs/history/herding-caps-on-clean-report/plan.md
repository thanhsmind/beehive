...
|---|-------|-------|--------|-------------------|
| 1 | The agent contract hands the worker an unrunnable finish command. | read | `packages/bee/agents/bee-build.md.tmpl:15` | `then \`bee cells finish --report\` — it runs NO tests; it records the proof line you hand it` — no `--id`, no `.bee/bin/` prefix |
...
```json
{
  ...
  "stages": [
    ...
    {"stage": "documentation-and-capture", "classification": "conditional", "role": "docs", "reason": "Only if a verify-app feature file needs the recipe."},
    {"stage": "read-only-gather", "classification": "conditional", "role": "read", "reason": "The two files are already read and anchored."},
