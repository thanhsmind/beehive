...
Publish 2.41.0 from `main`. One command does the whole release; this plan exists
...
|---|-------|-------|--------|-------------------|
| 1 | The release refuses without a deployment permit | ran | `scripts/release.sh 2.41.0` run at 2026-09-16T12:10Z | `release  FAIL: release authorization required — BEE_DISPATCH_ID is unset (dispatch with deployment stage under deploy role required); nothing was changed` |
...
| 5 | The version moves from the released 2.40.0, and 12 commits await the push | ran | `.claude-plugin/plugin.json`, `git describe --tags --abbrev=0`, `git log --oneline origin/main..HEAD \| wc -l` at 12:12Z | `"version": "2.40.0"`; latest tag `v2.40.0`; `12` |
| 6 | The work this release ships is capped, judged and proven | ran | scoped cell proofs for harness-worktree-relocation plus a live drive of the built binary | worktree `275 passed; 0 failed`, registry `12` + `9` passed, Pi `70 passed; 0 failed`, OpenCode `7 passed; 0 failed`; `bee worktree merge` from main with the session in the worktree left main at `055f9e99b` and printed `exit-worktree-before-merge` |
...
```json
{
  ...
  "stages": [
    {"stage":"deployment","classification":"required","role":"deploy","reason":"This lane exists to publish 2.41.0; the deploy role owns the release permit."},
