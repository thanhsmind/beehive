promote proposal for work item "release-red-fixes" (docs/history/release-red-fixes/plan.md) — 1 capped cell(s): rrf-1
anchor: history — docs/history/release-red-fixes/plan.md
PROPOSAL ONLY — nothing was written. Applying any section below is a human or agent decision.

(a) DELIVERY DRAFT — save as docs/knowledge/work/release-red-fixes/delivery.md
...
bee:
  ...
  sources: [docs/history/release-red-fixes/plan.md, .bee/cells/rrf-1.json]
---
...
areas: from the scribing stamp for "release-red-fixes" — .bee/logs/scribing-runs.jsonl's most recent entry (2026-09-15T06:58:23.997Z), the work item declares no bee.areas.
...
from cell rrf-1 — save as docs/knowledge/patterns/release-red-fixes-rrf-1-pitfall.md

---
type: bee.pattern
title: release-red-fixes cell rrf-1 — pitfall candidate
description: "Pitfall candidate mined from cell rrf-1's capped trace: rrf-1 escalated to the session model: the dispatched agy-flash worker never started because DNS failed for every host, so the leader made the edits inline"
timestamp: 2026-09-15
bee:
  id: release-red-fixes-rrf-1-pitfall
  lifecycle: draft
  areas: [onboarding]
  sources: [.bee/cells/rrf-1.json]
  polarity: pitfall
---

# release-red-fixes cell rrf-1 — pitfall candidate

## What the cell did

the three release-gate reds are green without changing release behavior

## Recorded evidence (verbatim from .bee/cells/rrf-1.json)

- **deviation** — rrf-1 escalated to the session model: the dispatched agy-flash worker never started because DNS failed for every host, so the leader made the edits inline
- **deviation** — the cli_shape change grew from one pinned exception entry to that entry plus counting distinct pinned exceptions hit, because release-red-fixes plan.md quotes the pinned span again inside its fenced cells JSON and occurrence counting read that as a dead exception; still test-only code, the recorded judge_obligation_ack holds
- **deviation** — sync-ack: skills owned by the touched areas never document the CLI spelling test's exception list, the gitignore template entries, or the work_verbs test env; rg over the owning skills finds none, so no owned skill text goes stale
