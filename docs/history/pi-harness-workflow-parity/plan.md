...
Waves: cells 1, 2, 3, 5, and 6 can start in parallel because their source files do not overlap. Cell 4 follows cells 2 and 3 because it binds plan packets to both dispatch and cap contracts. Cell 7 follows all code cells and runs generated-file parity. Cell 8 is a fix-first cell after the full suite exposed ambient identity leakage, a missing registry flag, and cross-worktree Cargo artifact reuse.
...
[
  ...
  },
  {
    ...
    "affects_skills": [],
    "affects_specs": [],
    "action": "Keep product Pi session lookup and herding activity routing unchanged. Capture process-start Pi and herding-worker identities without a first-call race, so in-process tests ignore inherited outer identities while explicit test identities still work. Isolate every sessionless black-box subprocess in the failed targets and shared fixture set unless a test supplies its own identity. Align proof and gate fixtures with the approved exact-proof and preview contracts. Declare preview in both gate registry entries and extend deletion coverage. Pin the proof CARGO_TARGET_DIR to this worktree, so another checkout cannot replace the executable under review. Do not clear Pi or herding identity variables. Make the previously red full suite pass in both the leader and dispatched-reviewer environments.",
    "verify": "CARGO_TARGET_DIR=\"$PWD/.bee/tmp/pihp8-cargo-target\" PATH=\"${CARGO_HOME:-$HOME/.cargo}/bin:$PATH\" cargo test --release --no-fail-fast --manifest-path packages/bee-rs/Cargo.toml",
    "must_haves": {
      "truths": ["Two consecutive full-suite runs pass from live Pi sessions, including a dispatched reviewer, against one worktree-local Cargo target", "Product commands still resolve PI_SESSION_ID after Bee and Claude identities", "Unit and black-box fixtures ignore only inherited Pi and herding-worker identities without first-call order dependence", "Explicit herding activity tests still route to their named job mailbox", "Proof and gate fixtures obey the new product contracts", "Both gate registry entries declare preview"],
      "artifacts": [{"path": "packages/bee-rs/crates/bee/tests/workflow_verbs.rs", "substantive": "removes ambient Pi identity from isolated black-box commands"}, {"path": "packages/bee-rs/crates/bee/src/hooks/activity.rs", "substantive": "distinguishes inherited worker identity from explicit herding activity fixtures in test builds"}],
      "key_links": ["the exact proof command uses a worktree-local Cargo target while retaining ambient Pi and herding identities", "test command helpers remove only inherited PI_SESSION_ID while Pi-specific integration tests set their own value", "activity tests ignore the process-start herding job while fire_herded identities still reach the mailbox sink", "registry payload matches run_gate accepted flags", "fixture proof commands and gate previews match their production contracts"],
      "prohibitions": ["Do not remove or clear PI_SESSION_ID from proof or production lookup", "Do not clear BEE_HERDING_WORKER or change production herding activity routing", "Do not reduce full-suite coverage", "Do not bypass exact proof or gate preview", "Do not hide command failures"]
    },
...
| Environment | Run Pi-only, Claude-only, Bee-only, mixed, absent, ambient test-runner, and dispatched herding-worker environments against a worktree-local Cargo target. | pihp-1, pihp-7, pihp-8 | Product precedence and herding routing stay ordered, isolated fixtures do not inherit outer identities, and another checkout cannot replace proof artifacts. |
