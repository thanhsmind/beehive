promote proposal for work item "slp-dissent-stop-and-ask" (docs/history/slp-dissent-stop-and-ask/CONTEXT.md + docs/history/slp-dissent-stop-and-ask/plan.md) — 6 capped cell(s): sd-1, sd-2, sd-3, sd-4, sd-5, sd-6
anchor: history — docs/history/slp-dissent-stop-and-ask/CONTEXT.md, docs/history/slp-dissent-stop-and-ask/plan.md
PROPOSAL ONLY — nothing was written. Applying any section below is a human or agent decision.

(a) DELIVERY DRAFT — save as docs/knowledge/work/slp-dissent-stop-and-ask/delivery.md
...
from cell sd-1 — save as docs/knowledge/patterns/slp-dissent-stop-and-ask-sd-1-pitfall.md

---
type: bee.pattern
title: slp-dissent-stop-and-ask cell sd-1 — pitfall candidate
description: "Pitfall candidate mined from cell sd-1's capped trace: Spelled the claim flag --reason instead of --claim — `claim` is in the CLI-wide FLAG_ALONE_BOOLEANS set (dispatch prepare --claim), so --claim <text> swallows …"
timestamp: 2026-08-28
bee:
...
# slp-dissent-stop-and-ask cell sd-1 — pitfall candidate
...
from cell sd-2 — save as docs/knowledge/patterns/slp-dissent-stop-and-ask-sd-2-pitfall.md

---
type: bee.pattern
title: slp-dissent-stop-and-ask cell sd-2 — pitfall candidate
description: "Pitfall candidate mined from cell sd-2's capped trace: The router coverage entry joined the cells dissent line that sd-1 added, as `cells dissent|dissent-verdict`, instead of the judge line the cell named — that de…"
timestamp: 2026-08-28
bee:
...
# slp-dissent-stop-and-ask cell sd-2 — pitfall candidate
...
- **deviation** — The verify chain ran as `export PATH=...; cargo test ...` instead of the inline `PATH=... cargo test ...` prefix — the worktree write guard refuses an inline env-var prefix as unverifiable — hit an unforeseen obstacle
- **deviation** — sync-ack: the cell file list names no skill: the worker-facing prose for dissent and StopAndAsk is Phase 4 of this same plan, and sd-2 ships only the orchestrator verb plus its declaration surfaces

## Status

Candidate only. `bee knowledge promote` proposes; naming the pattern, generalizing it beyond this cell, and moving `bee.lifecycle` to `active` are a human or agent decision.

from cell sd-3 — save as docs/knowledge/patterns/slp-dissent-stop-and-ask-sd-3-pitfall.md

---
type: bee.pattern
title: slp-dissent-stop-and-ask cell sd-3 — pitfall candidate
description: "Pitfall candidate mined from cell sd-3's capped trace: Extended the existing sweep instead of writing a sibling law, and added one sibling BITE test — the sweep took a third source in three lines, but a green sweep…"
timestamp: 2026-08-28
bee:
...
# slp-dissent-stop-and-ask cell sd-3 — pitfall candidate
...
from cell sd-4 — save as docs/knowledge/patterns/slp-dissent-stop-and-ask-sd-4-pitfall.md

---
type: bee.pattern
title: slp-dissent-stop-and-ask cell sd-4 — pitfall candidate
description: "Pitfall candidate mined from cell sd-4's capped trace: Generalized both guard.rs listers to Option<&str> and passed Some(capped) at the 5 existing call sites, instead of adding a skip-the-filter sibling — the rippl…"
timestamp: 2026-08-28
bee:
...
# slp-dissent-stop-and-ask cell sd-4 — pitfall candidate
...
- **deviation** — Could not author the tests red-first in order, since the arm had to compile before its cases could be written; reproduced the pre-change state instead by renaming the door key, ran the 8 new cases (all 8 FAILED), restored the key and re-ran green — hit an unforeseen obstacle
- **deviation** — The commit was refused mid-cell by the intake gate, which read the default store record a concurrent lane had left at phase idle while this lane own record said swarming; stopped and reported rather than working around the guard, and committed once the orchestrator reset that record — hit an unforeseen obstacle
- **deviation** — sync-ack: The worker-contract skill lines are Phase 4 of the approved plan (docs/history/slp-dissent-stop-and-ask/plan.md): Phase 4 gives skills/bee-swarming the dissent verb line, the boundary signals, and the options/leaning form. Phase 2 is the close door only, and editing that skill here would collide with the Phase 4 cell.

## Status

Candidate only. `bee knowledge promote` proposes; naming the pattern, generalizing it beyond this cell, and moving `bee.lifecycle` to `active` are a human or agent decision.

from cell sd-5 — save as docs/knowledge/patterns/slp-dissent-stop-and-ask-sd-5-pitfall.md

---
type: bee.pattern
title: slp-dissent-stop-and-ask cell sd-5 — pitfall candidate
description: "Pitfall candidate mined from cell sd-5's capped trace: followed the plan"
timestamp: 2026-08-28
bee:
...
# slp-dissent-stop-and-ask cell sd-5 — pitfall candidate
...
from cell sd-6 — save as docs/knowledge/patterns/slp-dissent-stop-and-ask-sd-6-pitfall.md

---
type: bee.pattern
title: slp-dissent-stop-and-ask cell sd-6 — pitfall candidate
description: "Pitfall candidate mined from cell sd-6's capped trace: Put the orchestrator verdict duty in swarming-reference.md beside the per-result sentence, not the SKILL.md rescue ladder — the cell offered either; the per-re…"
timestamp: 2026-08-28
bee:
...
# slp-dissent-stop-and-ask cell sd-6 — pitfall candidate
...
- **deviation** — Extracted result_envelope out of emit_result and asserted the value it returns — emit_result printlns straight to stdout, so there was no seam to assert without it; the cell named this as the smallest honest seam — followed the plan
- **deviation** — sync-ack: The bee-herding skills never enumerate the mailbox result schema (checked: no files_changed/proof/status field list under skills/bee-herding/), so nothing there went stale. 6a6b9975 scoped herding-lane dissent out of this feature: herding gets the three Rust surfaces plus one brief sentence, and the whole worker-contract half of StopAndAsk belongs to skills/bee-swarming/, which this cell updates.

## Status

Candidate only. `bee knowledge promote` proposes; naming the pattern, generalizing it beyond this cell, and moving `bee.lifecycle` to `active` are a human or agent decision.

knowledge promote: 6 capped cell(s) mined, 1 delivery draft, 15 area bullet(s), 6 pattern candidate(s), 0 file(s) written.