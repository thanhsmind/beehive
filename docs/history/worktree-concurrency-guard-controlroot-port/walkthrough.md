...
- Independent review session `worktree-concurrency-guard-controlroot-port-review-20260726`: 7 specialist reviewers (code-quality, architecture, test-coverage, api-contract, reliability, security, plus a delta re-review of the fail-open fix). **0 P1 across every reviewer and the delta pass.** 2 P2 (one fixed — the fail-open finding; one filed as tooling friction — see Known Limitations). 19 P3 filed to backlog or recorded advisory, none blocking. The security review confirmed self-exclusion, fail-closed-on-error (including the strict-mode propagation), and controlRoot scoping all hold correctly, with no new injection surface and no secret exposure.
...
- **Security reviewer dispatch was unreliable in the background** — 3 consecutive background dispatches (2 as a general-purpose type, 1 as the `bee-review` type) went idle without ever delivering report text, despite repeated direct pings; a 4th attempt, run synchronously in the foreground instead of as a mailbox-relayed background agent, succeeded immediately and returned a complete report (0 P1, 0 P2, 3 advisory P3). Filed as tooling friction (P2, `docs/backlog.md`) — worth investigating why background dispatch specifically failed 3/3 times for this one review lens. The eventual review result is real and complete, not worked around.
- 19 P3 findings filed to backlog or recorded in the review session (stale comment, duplicate test-row numbering, 3 real-but-narrow test-coverage gaps, message/doc drift in the old feature's walkthrough, 3 security-advisory notes on submodule-registration trust and fail-closed availability edges) — none blocking, all traceable to `worktree-concurrency-guard-controlroot-port` in `docs/backlog.md` or the review session record.
