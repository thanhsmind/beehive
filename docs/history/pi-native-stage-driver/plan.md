...
| 9c | `--seat`, the 600 s hat clamp and `detached_delivery` all live INSIDE the herding payload branch — which is why D11 keeps the work there. **(new)** | read | `packages/bee-rs/crates/bee/src/verbs/drivers/prepare.rs:2464` | `if runtime == "pi" && kind != "cell" && role.is_some()` (seat); `configured if runtime == "pi" && hat_seat.is_some()` (clamp); `if runtime == "pi"` (detached) |
| 10a | `doctor` embeds the belt at compile time and byte-compares the on-disk file. **(corrected anchor)** | read | `packages/bee-rs/crates/bee/src/doctor.rs:305` | `include_str!` at `:47`; the compare is `let same = on_disk.as_slice() == PI_EXTENSION_SOURCE.as_bytes();` at `:305` |
...
| 11a | The contract suite parses the belt source and derives its `pi.on` and `registerCommand` sets. | read | `packages/bee-rs/crates/bee/tests/pi_plugin_contracts.rs:288` | `include_str!` at `:107`; `const MARKER: &str = "pi.on(\"";` at `:288` |
...
| 13c | The fix has TWO homes, not one: the state branch compares the active feature, and the lane branch compares nothing at all. **(corrected)** | read | `packages/bee-rs/crates/bee/src/verbs/state_group/plan_packets.rs:632` | the lane branch returns `approved_cell_packet` with no feature comparison; the state branch guards only on `m.get("feature") == Some(feature)` |
| 14 | A herding worker runs with a muted hook posture that a belt-spawned child would not have had — the reason D11 keeps the spawn on this path. **(new)** | read | `packages/bee-rs/crates/bee/src/hooks/mod.rs:91` | `pane_env.insert("BEE_HERDING_WORKER".to_string(), "1".to_string());` and `std::env::var("BEE_HERDING_WORKER").is_ok_and(\|v\| !v.is_empty())`, with `:145-146` muting every hook but `activity` |
| 15 | `herding.agents` carries TWO config shapes, so "is this a `pi` binary" cannot be `agents[name][0]`. **(new)** | ran | `.bee/config.json` | `pi-gpt-5.6-luna  ARRAY  argv0='pi'` versus `agy-flash  DICT  argv0='agy'` (argv nested under an `argv` key) |
