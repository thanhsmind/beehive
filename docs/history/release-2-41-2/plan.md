...
Publish 2.41.2 from `main`. One command does the whole release; this plan exists
...
| 5 | The version moves from the released 2.41.1 | ran | `.claude-plugin/plugin.json` | `"version": "2.41.1"` |
| 6 | The work this release ships is capped, judged and proven | ran | `docs/history/pi-statusline-cache-by-model/CONTEXT.md`, cell psc-1 | full `cargo test --release` suite `passed 4075 failed 0`; judge verdict PASS on psc-1 (confidence=high); live Pi contract test drive of the model-usage status behavior |
...
[
  {
    ...
    "title": "Publish release 2.41.2 through the sanctioned script",
    "lane": "tiny",
    "role": "deploy",
    "deps": [],
    "decisions": [],
    "files": [".claude-plugin/plugin.json", ".codex-plugin/plugin.json", "docs/history/codex-harness-hardening/release-manifest.json"],
    "read_first": ["docs/history/release-2-41-2/CONTEXT.md", "scripts/release.sh"],
    "affects_skills": [],
    "affects_specs": [],
    "action": "Run `scripts/release.sh 2.41.2` with BEE_DISPATCH_ID set to an authorized deployment dispatch. The script bumps both plugin manifests, runs the regen chain (render-skill-trees, onboard --apply, release-manifest --write), runs the declared test suite before tagging, makes the release commit, tags v2.41.2, pushes main and the tag, waits for the release-binaries workflow and verifies the published assets. Never walk those steps by hand. Re-running at a version already committed is idempotent — it resumes the tail. The regen obligation on .claude-plugin/plugin.json is discharged INSIDE the script's own regen chain, which is why the release manifest rides this cell's files.",
    "verify": "bash scripts/release.sh 2.41.2 && bee dev release-manifest --check",
    "must_haves": {
      "truths": [
        "both plugin manifests report 2.41.2",
        "tag v2.41.2 exists locally and on the remote",
        "the GitHub release for v2.41.2 carries the binaries and SHA256SUMS",
        "the script printed its final OK line"
      ],
      "artifacts": [
        {"path": ".claude-plugin/plugin.json", "substantive": "version 2.41.2"},
        {"path": ".codex-plugin/plugin.json", "substantive": "version 2.41.2"}
      ],
...
```json
{
  ...
  "stages": [
    {"stage":"deployment","classification":"required","role":"deploy","reason":"This lane exists to publish 2.41.2; the deploy role owns the release permit."},
