...
Publish 2.45.0 from `main`. One command does the whole release; this plan exists
...
| 5 | The version moves from the released 2.44.0 | ran | `.claude-plugin/plugin.json` | `"version": "2.44.0"` |
| 6 | The work this release ships is capped, judged, leader-checked and closed | ran | `bee close --feature mistake-fix-at`, `bee close --feature no-code-comments` | `Tests GREEN for "no-code-comments" — 4 capped cell(s) all carry a proof line`; both closes retired their cells, and the script runs the full declared suite again before tagging |
| 7 | The release expects five binaries | read | `scripts/release.sh:345` | `[ "$BIN_COUNT" -ge 5 ]` |
...
```json
{
  ...
  "stages": [
    {"stage":"deployment","classification":"required","role":"deploy","reason":"This lane exists to publish 2.45.0; the deploy role owns the release permit."},
