...
Publish 2.44.0 from `main`. One command does the whole release; this plan exists
...
| 5 | The version moves from the released 2.43.0 | ran | `.claude-plugin/plugin.json` | `"version": "2.43.0"` |
| 6 | The work this release ships is capped, leader-checked and proven | ran | `docs/history/host-packaging-gaps/plan.md`, cells hpg-1..hpg-4 | each cell carries an ok leader check; the full declared suite ran green (4205 passed) before merge, and the script runs it again before tagging |
| 7 | The release now expects five binaries, built on three runners that have never run | read | `scripts/release.sh:345` | `[ "$BIN_COUNT" -ge 5 ]` |
...
```json
{
  ...
  "stages": [
    {"stage":"deployment","classification":"required","role":"deploy","reason":"This lane exists to publish 2.44.0; the deploy role owns the release permit."},
