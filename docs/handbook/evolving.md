...
```text
Guard (bee repo?) -> Rank feedback -> Gate A (human picks) -> Fix via the writing-skills
discipline -> Suites green -> Gate B (human reviews diff) -> Push (named, manual)
```
...
Hand the chosen item to the **writing-skills discipline**
([docs/handbook/writing-skills.md](writing-skills.md)) and follow it in full (no
mechanical-edit exemption exists): failing pressure test recorded FIRST, then the minimal change,
then re-test GREEN. The evolving loop never implements inline — it conducts the loop, not the
edit. A fix touching non-skill surfaces still enters the normal bee chain (cells, verification,
capping).
...
- this loop running from a trigger, schedule, or another agent's dispatch instead of the human
