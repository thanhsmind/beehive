...
| p-9fdc119a | Worker brief carries an expertise/context section: the knowledge-area docs and skill references the task needs, so the worker knows what capabilities to apply | 1) Herding run brief (render_brief) gains a Context/Expertise section listing repo-relative doc paths (knowledge-area files, skill reference files) relevant to the task — files to READ, never bee commands, so the standalone-executor contract (herding-worker-standalone D1-D3, worker stays bee-ignorant) holds. 2) The same section appears in the bee-swarming worker brief for Task-tool workers where it can name skills directly. 3) Source of the list: the cell/backlog item's declared knowledge context (bee knowledge context manifest for the feature) plus any explicitly attached refs; empty list renders nothing. 4) Round-trip proven: a dry-run brief shows the section; a worker result cites one of the listed docs. | proposed | — |
