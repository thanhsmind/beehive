...
| Cells cap through the pending path by default; the routine wave-close impacted run and routine per-`[DONE]` verify re-run are retired, not relocated; re-run stays a smell-triggered judgment call | main-verifies D1/D4 | Per-cell/per-wave proof is deliberately relocated to the feature boundary; the delegator checks the work at the shippable unit |
| ONE feature verify runs at final-slice close, recorded via `state feature-verify record`; the close door (`guardFeatureVerifyDebt`) refuses leaving `swarming`/running `scribing-run` while a pending cap lacks a newer green record, no `gate_bypass` level lifts it | main-verifies D2/D3 | The relocated proof needs a relocated, mechanically enforced gate, or the law is prose |
| Red feature-verify → fix cells in the same feature, never un-cap; bugfix repro red is MAIN-produced pre-dispatch, cited in the cell, never worker re-proved | main-verifies D5 (et-4 precedent) | Already the slice-red law, now at feature granularity |
| `bee-scribing` invokes only on the feature's FINAL slice close; a non-final slice returns to bee-planning with no scribing call | decision 7346e9d7 (feature-close philosophy) | Scribing and compounding are feature-close events, never per-slice/per-execution-round |
