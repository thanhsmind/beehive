...
- **Recording a route while any worktree grant existed (counter-teeth ct-1,
  2026-08-04).** The third instance of the same shape, and the most expensive:
  recording a triage for a code-touching lane bailed outright whenever *any*
  grant was live, and the bail surfaced as an argument-shape complaint — a
  refusal that named nothing true about the situation. Because the route record
  is the remedy every route-less refusal points at, the broken verb made its own
  remedy unreachable, and the feature that wanted to tighten the route-less
  claim had to fix this first. Ported, the branch is ordinary: a grant held by
  the feature being triaged turns the block into *continue there*, naming the
  existing worktree and the merge command; a grant held by any other feature is
  invisible to the call and changes nothing about it. No path emits the
  argument-shape complaint any more.
...
- Route recording under a grant (the third un-ported arm): `route_worktree_block`
  in `packages/bee-rs/crates/bee/src/verbs/state_group/workflows.rs:638-674`,
  called from `run_route` (`workflows.rs:823`); the feature-scoped lookup is
  `find_granted_worktree_for_feature` (`topology.rs:198`), which is what makes a
  foreign feature's grant invisible. Tests
  `route_worktree_block_ignores_a_foreign_features_grant` and
  `route_worktree_block_never_declines_under_any_grant_shape`
  (`state_group/tests.rs:1347,1395`). Evidence: trace `.bee/cells/archive/counter-teeth/ct-1.json`,
  commit f6398f8e (state_group 48 passed, 0 failed, 2026-08-04).
