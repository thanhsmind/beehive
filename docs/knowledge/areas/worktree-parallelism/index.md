...
- [Worktree Parallelism — returning: the staged merge and its verify gate](returning-and-the-merge-gate.md) — Why a feature worktree returns through a merge that is staged but never committed until the configured verify passes, why the coordination lock releases around that verify child and re-acquires behind a four-part fence before any commit, how a textual conflict, a red verify, and fence drift all abort while proving main untouched, and when post-commit cleanup runs and when it refuses.
