...
description: "The prose routing rule that sends new feature work into a worktree only when an occupied checkout makes it worth it, the lane-first refinement that defers the grant to Gate 2's execution component (the old standalone Gate 3, folded in), and the notices an ungranted worktree and a denied write print so isolation is never silently absent."
...
old standalone Gate 3, folded into Gate 2 — validation-diet D2), and only when the feature's
execution genuinely overlaps files with other in-flight work. Most parallel work never needs the
worktree at all;
