...
- [Rust Runtime — measuring text: characters for what people read, code units for what a foreign contract counts](text-measurement-and-the-two-counting-units.md) — Why this runtime measures string length two different ways on purpose, which one every display and log cap uses, the two narrow places that keep the other one and what each of them is pinned to, and how the seven scattered copies of that logic became one module with a single rule per caller.
