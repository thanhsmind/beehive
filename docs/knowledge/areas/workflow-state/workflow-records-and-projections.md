...
guarded helper (`close_workflow_records`) that preflights all matched targets
before any mutation: if any target workflow holds an open `planned-next`
handoff record in its mailbox, the whole close operation refuses (typed, zero
mutation across any workflow or handoff record) to protect open claim authority.
When clear to proceed, any open `pause` handoffs across the target workflows are
marked `'cleared'` in their respective mailboxes (with audit records preserved on
disk), target workflow records transition to status `"closed"`, and projections
are rebuilt. In addition, projection rebuilds
(`rebuild_handoff_projection_reporting`) explicitly exclude workflows with status
`"closed"` as crash-recovery defense against stale projections.
