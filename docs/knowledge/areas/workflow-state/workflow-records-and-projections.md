...
explicitly can close it, which is the deliberate escape. All three workflow
close selectors (`--id`, `--feature`, `--all-but-active`) run through one
guarded helper (`close_workflow_records`) that linearizes with mailbox writers
under the stable lock order `workflow:<id>` then `handoff:<id>` (D3, pprf-3).
Target workflow IDs are sorted and deduplicated, and both locks are acquired
for each target in ascending order before inspection or mutation begins.
With all locks held, `close_workflow_records` preflights all matched targets:
if any target workflow holds an open `planned-next` handoff record in its
mailbox, the whole close operation refuses (typed, zero mutation across any
workflow or handoff record) to protect open claim authority. When clear to proceed,
any open `pause` handoffs across the target workflows are marked `'cleared'` in
their respective mailboxes using `clear_mailbox_pause_handoffs_assuming_lock`
(with audit records preserved on disk), target workflow records transition to
status `"closed"` via `update_workflow_assuming_lock`, and all locks are released
before projections are rebuilt. In mirror fashion, `write_mailbox_handoff`
acquires `workflow:<id>` before `handoff:<id>` and refuses under lock if the
workflow record is missing, unreadable, or not active (`status != "active"`).
In addition, projection rebuilds (`rebuild_handoff_projection_reporting`)
explicitly exclude workflows with status `"closed"` as crash-recovery defense
...
- R105 — A feature's workflow record is created through one idempotent seam
  ...
  feature refuses rather than closing everything; all close selectors preflight
  targets to refuse if any target has an open planned-next handoff, clear open
  pause records while preserving mailbox history, and rebuild projections;
  projection rebuilds ignore closed workflows (workflow-lifecycle, cells wl-1/wl-2,
  2026-08-05; pi-full-workflow-parity pfp-1, decision 0510d81d).
...
- Workflow close handoff coordination and projection filter (R105, pfp-1):
  `close_workflow_records` in
  `packages/bee-rs/crates/bee/src/verbs/state_group/workflows.rs`;
  `clear_mailbox_pause_handoffs` in
  `packages/bee-rs/crates/bee/src/verbs/workflow_store/handoff.rs`;
  closed workflow filter in `rebuild_handoff_projection_reporting` in
  `packages/bee-rs/crates/bee/src/verbs/workflow_store/projections.rs`. Evidence:
  cell `pfp-1`, decision 0510d81d.
- Workflow close and mailbox write linearization (R105, pprf-3, decision 7da86fdb):
  `close_workflow_records` in
  `packages/bee-rs/crates/bee/src/verbs/state_group/workflows.rs` locks targets
  in sorted `workflow:<id>` then `handoff:<id>` order, holding locks across
  preflight, pause clearance, and status transition; `write_mailbox_handoff` in
  `packages/bee-rs/crates/bee/src/verbs/workflow_store/handoff.rs` acquires
  `workflow:<id>` before `handoff:<id>` and validates active workflow status
  before writing. Evidence: cell `pprf-3`, decision 7da86fdb-40de-415c-b3ac-ae84a4c89d60.
