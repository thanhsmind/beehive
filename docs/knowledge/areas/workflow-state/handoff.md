...
**B48 — Safe pause handoff dismissal clears the open record, preserves mailbox history, and rebuilds projections (pi-full-workflow-parity pfp-1, decision 0510d81d).** Trigger: `bee state handoff dismiss`. What happens: the newest open pause handoff record in the target workflow's mailbox is marked `'cleared'` with a `cleared_at` ISO-8601 timestamp and written back atomically. The mailbox audit file on disk is never deleted. Rebuilding the compatibility projection (`rebuildHandoffProjection`) removes or updates the legacy `.bee/HANDOFF.json` projection. If the newest open handoff is `planned-next`, dismissal is refused outright (typed, zero mutations on claim or handoff) because planned-next handoffs carry claim authority and must be adopted via `bee state handoff adopt` (D1). In C1 legacy mode (zero workflow records), `state handoff dismiss` removes `.bee/HANDOFF.json` for pause records and refuses planned-next.
...
- R98 — `bee state handoff dismiss` marks only open pause handoff records `'cleared'`,
  preserves mailbox files on disk for audit history, and rebuilds the legacy
  compatibility projection; planned-next handoffs refuse dismissal without
  mutating claims or handoff records (pi-full-workflow-parity pfp-1, decision
  0510d81d).
- R99 — Mailbox writes and workflow close linearize under workflow:<id> before handoff:<id>:
  `write_mailbox_handoff` validates workflow state under lock and refuses closed, missing,
  or unreadable workflow records with zero mutation; close preflight, pause cleanup, and
  status writes hold both locks across all targets (pi-parity-review-fixes D3, decision
  7da86fdb).
...
- Recorded start manner and the adoption door (B47/R97): the `source` field on
  ...
  `handoff.rs:350-357`. Evidence: trace `.bee/cells/archive/hook-teeth/bh-4.json` (workflow_store
  38, session_init 17, all green, 2026-08-04).
- Safe pause dismissal (B48/R98): `dismiss_mailbox_handoff` and
  `clear_mailbox_pause_handoffs` in
  `packages/bee-rs/crates/bee/src/verbs/workflow_store/handoff.rs`; C1
  fallback `dismiss_handoff` in
  `packages/bee-rs/crates/bee/src/verbs/state_group/store.rs`; CLI handler
  `run_handoff_dismiss` in
  `packages/bee-rs/crates/bee/src/verbs/state_group/sessions.rs`; dispatcher
  routing in `packages/bee-rs/crates/bee/src/verbs/state_group/set_gate.rs`.
