...
- [Widening a shared vocabulary trips the guard that pins the prompt to the verb](../../patterns/slp-advisor-nudge-an-1-pitfall.md) — Widening a closed set like KNOWN_SIGNALS in the verb requires the same-change prompt update the guard is built to demand
- [A door's refusal form follows its sibling refusals, not the plan's prose template](../../patterns/slp-advisor-nudge-an-3-pitfall.md) — Mixing a multi-line refusal template into a door whose other refusals are single typed lines reads worse than matching the sibling
