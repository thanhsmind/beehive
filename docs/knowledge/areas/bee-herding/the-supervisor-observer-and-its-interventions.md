...
(`mailbox::ACTIVITY_FRESHNESS_SECS = 120`). `bee herding status` and the
`bee herding run` poll tick share this exact constant: a job whose activity file
is older than 120 s while its pane process is alive projects status word
`stalled`. When fresh activity resumes, it projects `recovered` once before
returning to `working`. Note that this execution stall is distinct from herdr's
prompt stall (the prompt-delivery width stall described in
`the-run-verb-and-worker-outcomes.md`): status lines always prefix output with the
job id (`herding: job <id> stalled`) so the transport-level prompt stall and the
worker heartbeat stall never share a line.
