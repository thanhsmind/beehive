...
| The trailing test cell's first mandated step is a coverage judgment (`file:line` citations), not authoring; a test cell that authors no test is not a defect | worker-conformance D4 | Makes "do we need more tests?" the required thought instead of making "write tests" the required output; worked instance `docs/history/worker-conformance/reports/wc-3.md` |
| Test shape at `standard` and below is the triad; `edge-dimensions.md` applies only at `high-risk`/hard-gate | worker-conformance D5 | The 12-dimension matrix read as a checklist to fill, which is a volume generator at standard lane |
| Ratio ceiling, `new_suite_reason`, and the `refactor`+new-test-file refusal unchanged; no numeric per-group cap added | worker-conformance D6 | The triad is the shape guide, the ratio ceiling is the volume brake; two brakes on one axis would contradict |
| Doctrine-vs-machine gap recorded beside "bugfix/high-risk never batched": `testCellDebt` has no lane exemption, so a high-risk feature still plans one trailing test cell | worker-conformance D13 | Following the prose alone leaves the feature unable to leave `swarming`, with no bypass level lifting the refusal; the predicate is not fixed here |
