...
Cells run `commands.test` — the project's one declared test path — at finish (`bee cells finish` runs it and records `.bee/logs/test-results.json`; green caps, red refuses with the failing excerpt); `bee close` re-runs the same declared suite for the feature (`bee-swarming/references/swarming-reference.md`, "Tests at finish and close, in full"). `commands.verify` is the close/merge chain — `bee worktree merge` re-runs it on the merged tree — and CI owns the full estate: the full chain runs on the project's own CI cadence (push, nightly, or scheduled — the host workflow decides) and auto-files a `verify-red` issue when red; the release flow dispatches the CI full run (`gh workflow run CI --ref main`) right after the tag push, a red result arriving back as the same `verify-red` issue, not a local gate. A host keeps finish fast by pointing `commands.test` at its fast suite. Judges and reviewers verify against the diff and `must_haves`, never by running the full chain as part of a verdict.
