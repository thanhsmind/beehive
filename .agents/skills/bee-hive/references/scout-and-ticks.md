...
- `class` ∈ `feature`, `bugfix`, `docs`, `refactor`, `research`, `release`, `spike`, `perf`, `content`
  — each class has a playbook the plan cites by name and anchor, never copies:
...
Code-touching feature work is worktree-first (rule: agents-worktree-first) — mechanism in `docs/knowledge/areas/worktree-parallelism/routing-and-visibility.md`: the feature's worktree is created at feature start — `bee worktree new --feature <slug>`, then the next session opens in the printed path — by default, not only when the checkout turns out to be occupied. Release always runs in main; `--in-main` at feature start is the recorded owner override, never silent. Merge-back happens from main via `bee worktree merge --id <id>`; before the merge is even attempted, a zero-mutation D7/D8 proof check confirms every capped cell for the feature already carries a recorded proof line — a proof-less merge refuses (`WORKTREE_MERGE_PROOF_DEBT`) before main is touched at all. Past that check the merge is staged uncommitted (`git merge --no-ff --no-commit`); a textual conflict is the alarm to investigate, and it aborts the stage, leaving main byte-untouched, not a signal to roll back a commit (none was ever made). No verify command runs against the staged tree — CI runs the full declared suite against the merged main on every push, the one deterministic net.
