...
| Step | Rule |
|---|---|
| 1. Initialize | Read `AGENTS.md` and `CONTEXT.md`. Your dispatch prompt INLINES the cell's full JSON and the state-at-dispatch line (exec-speed D7) — both are authoritative; do **not** re-run `status --brief --json` and do **not** re-fetch the cell via `cells show` (the orchestrator copied both from its own fresh read at claim time; ownership is still enforced at cap by the claim guard). Use the parent-given nickname as your reservation identity. A dispatch prompt MISSING the inlined cell JSON is malformed → `[BLOCKED]` naming the gap, never a silent re-fetch. |
| 2. Accept cell | Exactly **one** parent-assigned id — never browse `ready`/`list` or self-select. Validate against the INLINED cell JSON (exec-speed D7): missing or already capped → `[NOOP]`. Ambiguous, deps not capped, or conflicts a locked `CONTEXT.md` decision → `[BLOCKED]`, never reinterpret to fit. **Validate, never claim:** the orchestrator already claimed it before spawning you — confirm `status: "claimed"` and `trace.worker` matches your nickname; anything else is not yours to touch → `[BLOCKED]`/`[NOOP]`, never claim it yourself. |
...
| 5. Commit | Implement, then commit — one commit per cell; the subject describes the change in imperative mood (never the process, the counts, or the cell), and the cell id rides the last line of the body as a trailer. `[DONE]` carries the diff and commit, never verify output. The cell's `verify` field is MAIN's command, not yours — never run it, never cite its output as evidence. Bugfix cells: the repro red is already MAIN-produced pre-dispatch and cited in the cell — fix it, never re-prove it. Classic path (sanctioned for spot use, other repos, or transition): run the cell's targeted `verify` command and record it with output before cap. Full matrix, amendments, test-shape rules, debug discipline: `references/worker-details.md` ("Verify in full"). |
| 6. Advisor consult | High-risk/hard-gate cells require a recorded advisor consult before the execution gate — capping throws without a fresh `advisor_ref` (staleness is hash-and-decision-anchored, never a TTL). Resolve the advisor from config, run it read-only, record via `state advisor-ref record`. This gate-time consult carries a COMPACT DIGEST on stdin (exec-speed D11): cell id, one-paragraph change summary, file list with a one-liner per file, and the `CONTEXT.md` path — never full file excerpts; the full evidence bundle (failing command, failing output, diagnosis, cited excerpts) belongs to the on-failure consult loop only. Advice never approves a gate and never overrides a locked decision. Full mechanics, digest shape, consult prompt: `references/worker-details.md` ("Advisor consult in full"). |
...
| 9. Return | Start the final message with exactly one of `[DONE]`, `[BLOCKED]`, `[HANDOFF]`, `[NOOP]`, then the result fields. A routine `[DONE]` writes NO report file (exec-speed D10, extending decision 0009's single-source-trace rule): the cap trace plus the status-token message are the record. A report at `docs/history/<feature>/reports/<cell-id>.md` is REQUIRED only for `[BLOCKED]`/`[HANDOFF]`, for any cell that carried Advisor Consults (with the **Consults** section: count, advisor identity, one-line ask/answer digest each — bee-swarming's goal-check reads this), or when the orchestrator explicitly asked for one. When written: status token, one-line outcome, files touched, a link to `.bee/cells/<cell-id>.json` for the full trace — never re-embed evidence JSON or verify output, never a separate scratch file. |
...
continuing — post-compaction `cells show --id <id>` is the one sanctioned
re-fetch (the inlined prompt copy may be gone; exec-speed D7).
...
A rule whose letter stops serving its purpose is not license to improvise:
a worker deviates only through the Deviation Rules above, recorded — anything
beyond them is `[BLOCKED]` with the reason, never a silent judgment call
(rails for workers; the Judgment contract's orchestrator latitude does not
apply inside a worker turn).

One status token returned (plus the report file when step 9 owes one —
exec-speed D10); the parent orchestrator collects it. Invoke bee-swarming
skill to continue the wave.
