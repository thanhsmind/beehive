...
17. **Progress ticks: one line per step, on by default.** Every perceivable pipeline step emits exactly ONE short chat line in work language — outcome, never mechanics. Format fixed: `<glyph> <event>: <what> — <key fact>`. In `tiny`/`small` lanes, consecutive GREEN ticks of one phase may composite into a single line (events comma-joined; exec-speed D9) — a `✗` line is never composited or delayed; `standard`/`high-risk` keep one line per step. **No bypass level, at any tier, ever silences a tick**, and a red or refusal is never silence-able. Only two switches produce silence: `quiet: true` in `.bee/config.json` silences the tick stream but never the `✗` red/refusal line, and `ship_visibility: "off"` silences only the two PR ticks (draft PR opened, demo posted). Worked example per step: `bee-hive` → `references/routing-and-contracts.md` ("Progress ticks").
