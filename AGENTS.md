...
15. **Concurrency is the default; serial is the exception, named.** If pieces of work can run at once, open the threads and run them: gather fans to I/O workers (rule 12), a slice's cells fan to a wave, and independent ready features fan to lanes (`bee state start-feature --as-lane --paths <paths>`) or worktrees. Serial only for a declared path overlap, a true dep, a scarce resource, or explicit human instruction. Full protocol: `bee-hive` → `references/routing-and-contracts.md`.
