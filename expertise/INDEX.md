...
Never load all of them — route, then read exactly one.
...
| [tests.md](tests.md) | Coverage audits, behavior vs structure, test levels and doubles, determinism, choosing cases, red-before-green, running the suite, plus indexed patterns under `tests/patterns/` | Writing or judging tests; a test flakes; fixing a reported bug; a bug shipped despite a green suite; matching a second implementation to an existing one |
| [review.md](review.md) | Findings, severity, adversarial reading, verification, evidence standards, scope, self-review | Reviewing a change; filing or disputing a finding; no fresh reviewer is available |
...
| [debugging.md](debugging.md) | Repro-first, error reading, hypotheses, instrumentation, bisection, environment, fix closure | A bug is in front of you; a fix "works" but is unexplained; a symptom looks familiar or impossible |
