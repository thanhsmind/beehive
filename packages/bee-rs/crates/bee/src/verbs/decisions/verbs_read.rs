...
/// path built by `path_relative`, forward-slash/normalized). Excludes the
...
pub(crate) fn touches_sweep_excluded(root_relative_file: &str, bound_feature: Option<&str>) -> bool {
    ...
    if normalized == "docs/decisions/index.md" {
        return true;
    }
    if let Some(feat) = bound_feature {
        if !feat.is_empty() && normalized.starts_with(&format!("docs/history/{feat}/")) {
            return true;
        }
    }
    false
