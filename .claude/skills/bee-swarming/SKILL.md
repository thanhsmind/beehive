...
After `[DONE]`, emit the cap tick (`bee-hive/references/routing-and-contracts.md`, "Progress ticks") and — never the worker — author the done-report: its evidence is the worker's verbatim diff plus the orchestrator's own independent verify re-run (AO14, decision 0018's goal-check restated as authorship, not new mechanics). Then hand off: both `tiny` and `small` present that done-report (diff + fresh verify output + capture line) and invoke bee-scribing — no auto reviewer; the 1-correctness-reviewer contract lives inside a user-invoked session (implementation is verified; independent review runs only on user request, R1).
