...
- **herding execution branch** — `bee herding run` is the cell-execution mirror of the cli gather
  ...
  D3). Boundary held exactly like `cli`'s: cell-execution-only, never a gather transport — a gather
  never dispatches through a herding pane (herding-executor D7, the mirror of the `for_gather` branch
