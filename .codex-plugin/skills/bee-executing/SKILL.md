...
| 6. Advisor consult | High-risk/hard-gate cells require a recorded advisor consult before the execution gate — capping throws without a fresh`advisor_ref` (staleness is hash-and-decision-anchored, never a TTL). Resolve the advisor from config, run it read-only, record via `state advisor-ref record`. This gate-time consult carries a COMPACT DIGEST on stdin: cell id, one-paragraph change summary, file list with a one-liner per file, and the `CONTEXT.md` path — never full file excerpts; the full evidence bundle (failing command, failing output, diagnosis, cited excerpts) belongs to the on-failure consult loop only. Advice never approves a gate and never overrides a locked decision. Full mechanics, digest shape, consult prompt: `references/worker-details.md` ("Advisor consult in full").                                                                                                                                                             |
...
re-fetch (the inlined prompt copy may be gone).
...
above; anything ambiguous becomes `[BLOCKED]` with an `Outstanding Questions` section in the report. Workers never approve gates — that
...
the parent orchestrator collects it. Invoke bee-swarming
