---
name: bee-technical-writing
description: >-
  The layered standard for technical documents: document mode, reader-facing sentences, one-reading instructions, and plain global English. Use when writing or reviewing a document, RFC, README, spec, plan, PR description or commit message. Not for chat replies, which bee-unslop covers, or product UI copy.
metadata:
  version: '0.1'
  ecosystem: bee
  dependencies: []
...
> Ported from pstack (cursor/plugins, pstack/skills/technical-writing), adapted for bee.
...
When the project's own instructions set a writing standard, a code style or a language, that standard wins where the two disagree, including over the tabs rule below.
...
- Apply the `bee-unslop` skill to every doc this skill touches. That skill owns the slop-pattern catalog: AI vocabulary, filler, hedging, formatting tells.
- Never rewrite quoted evidence, ids, commands or code.
...
A headless run applies this standard unchanged and never stops to ask about mode, tone or wording.

Text written to the standard. Invoke bee-hive skill.
