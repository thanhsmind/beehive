...
**Product-backlog (PBI) drift** — audit `node .bee/bin/bee.mjs backlog pbi list --json` (the fold over `.bee/backlog.jsonl` — the one unified store; friction/grooming events live in the same stream but are a separate `kind`) for three drift patterns, each a tiny fix cell (prose-ruled correction, never a hook):
...
Promote-to-check proposals (a repeated finding becoming a grep/lint/guard) ride the same template with `risk_lane` tiny or small; changing what a gate or gate-adjacent guard *blocks* is never tiny.
