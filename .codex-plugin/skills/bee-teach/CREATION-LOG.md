...
The leader reran scenario 4 through `bee dispatch prepare --kind advisor --brief-file`. The first RED run above used the `advisor` role (Fable), before the Fable quota ran out. To compare like with like, the leader ran S4 again with no skill and then with the skill, both on the `plan` role (Opus). The GREEN run loaded only `bee-teach`, through its read diet.

### RED on Opus (no skill, the same role as GREEN)

**Verdict:** FAILED.
- A metaphor in place of the mechanism: "It gets a new yes/no box called `trimmed`".
- Seven paragraphs, not the smallest complete answer first.
- A menu at the close: "You have two options: … Approve the plan as it is … Tell me to make it one job."

### GREEN on Opus (with bee-teach)

**Verdict:** PASS.
- It opens with a plain definition: "A cell is one small piece of the work. Each cell is built, tested, and saved on its own before the next one uses it."
- It names the mechanism, with no metaphor: "A field is one named value inside that record."
- It builds the picture one diagram at a time. The first diagram shows cell 1 alone. The second redraws cell 1 and adds cells 2 and 3.
- It closes on exactly one next action, the gate question itself: "Do you approve the plan as it is written?"
- It has no preview line and no menu.

**Result:** the same model, the same scenario and the same role give opposite verdicts. The skill fixes each observed failure.
...
- RED S4: FAILED on Fable and on Opus with no skill.
- GREEN S4: PASS on Opus with `bee-teach`.
