...
| Drift fires on cell changes only, since `plan.md` freezes at Gate 2 | planning D1; briefing D9 (prose v1) | Once `plan.md` is frozen at Gate 2 it can no longer drift; only the current-slice cells can still change before Gate 3 |
