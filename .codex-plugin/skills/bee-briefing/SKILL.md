...
One artifact per feature: `docs/history/<feature>/implement-plan.md`. Consolidates the truth artifacts (`CONTEXT.md`, `approach.md`, `plan.md`, cells, validating reports); authors only Technical Design and Rollback Plan. Never originates a decision, scope, approach, or cell — inventing content to fill a section is the one failure this skill exists to prevent. Rules stated bare — decision IDs: `references/provenance.md`.
...
| refresh (chain) | after Gate 2 prep + after validating | re-project changed sections in place, never a second file |
...
| Validation Plan | cell `verify` + validating report | describe + link evidence, never assert unrun |
...
- **Rollback Plan** — how *this* change is undone (revert commits / disable a flag / reverse a migration). Undecided → "OPEN QUESTION: …", never a plausible procedure nobody agreed to. `high-risk` must resolve it before Gate 3.
...
`bee-planning`/`bee-validating` present gates, not briefing — the brief is what their Gate 2/3 message **links**. Chat stays plain-language; the brief is the durable review object; never paste the whole brief into gate chat.
