...
name: bee-unslop
description: >-
  Remove the patterns that make prose read as machine-generated. Use when writing or editing user-facing prose — a reply, a document, a plan, a spec, a README, a PR description or a commit message — and when asked to make text read like a person wrote it. Not for progress ticks, red or refusal lines, command output, quoted evidence, ids, or code, which stay verbatim.
metadata:
  version: '0.1'
  ecosystem: bee
  dependencies: []
...
> Ported from pstack (cursor/plugins, pstack/skills/unslop), adapted for bee.
...
- Apply these rules to user-facing prose and saved documents.
- Never rewrite progress ticks, red or refusal lines, command output, quoted evidence, ids or code. They stay verbatim.
- When the project's own instructions set tone, length, style or language, they win where the two disagree.
- Reply in the user's language. Apply the English word lists only to English text.
...
A headless run applies these rules unchanged and never stops to ask about tone or wording.
