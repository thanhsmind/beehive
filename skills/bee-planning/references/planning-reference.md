...
`plan.md` is the truth artifact for **standard/high-risk** lanes (and small only when opt-in). Tiny drops it entirely and small skips it by default — the cell(s) and the logged scoping synthesis carry the shape. Where `plan.md` exists, discovery and approach content default to **sections inside it**; they graduate to their own files only when real complexity makes a standalone file worth reading on its own. A small/standard feature that spawns `discovery.md` + `approach.md` + `plan.md` + `implement-plan.md` restates the same "current state" four times.

| Artifact | Separate file when | Otherwise |
|---|---|---|
| `plan.md` | standard/high-risk (frozen at Gate 2), or small when a durable multi-slice/product-decision doc is genuinely needed | **tiny: none** — request + one cell; **small: opt-in** — a logged scoping synthesis + 1–3 cells is the default |
...
Frozen at Gate 2: once `approved_gates.shape` is set the content sections are immutable — the only permitted post-approval write is the approval stamp in the frontmatter. No requirements-only→implementation-ready mutation, no in-place enrichment.
...
No `## Current slice` / `## Cells` sections are added post-approval: the plan is frozen and the current slice lives **only in cells**. Prep creates the cells; the plan is never re-opened to index them.

**Shape bodies by mode:**

- `tiny` — no plan.md: the cell `action` is the micro-plan (current work outcome, proof command, out of scope).
- `small` — no plan.md by default: a logged scoping synthesis + 1–3 cells carry the shape; write a plan.md only when opt-in (durable multi-slice/product-decision doc).
...
2. **Bounded files.** `files` lists everything the worker may write; `read_first` what it must read. A worker touching other paths returns `[BLOCKED]`. Cross-cell file overlap is legal, not a scoping error — it only costs a wave (auto-serialized); prefer explicit paths or trailing-`*` patterns in `files`, since overlap detection treats mid-path globs as literals, not wildcards.
...
5. **behavior_change honesty.** Any cell changing observable behavior is `behavior_change: true`. Do not sell the flag as a universal cap refusal — it is not one. The "declares `behavior_change`, records no verification evidence" door is a **non-blocking warning** — the cap succeeds and the absence is recorded — and the `red_failure_evidence` "before" door fires **only where the required proof tier resolves to `red-first`** — `security`/`migration` in every lane, `bugfix`/`behavior`/`api` at lane `high-risk`. A `standard`-lane behavior cell resolves to `existing-targeted-green` and is refused by neither. That is not "no proof still caps": the classic path still refuses a cap without a recorded **passing** verify; what is merely recorded instead of refused is the pass asserted with nothing to show for it — `trace.proof: "unrecorded"`, which arms the feature close-door. What the flag decides is real: the proof tier, the scribing debt, and reviewing's scrutiny — mislabeling is a P1 waiting to happen.
...
Create with one batched stdin call for the whole slice (a JSON array; a single object works for a one-cell slice — do not write per-cell scratchpad files; the one canonical scratch home, if one is ever genuinely needed, is `.bee/tmp/<feature-or-session>/`):
...
It is a **read-only gather**, never a cell: a cli-shaped review slot resolves with the purpose-scoped `resolveTier(root, 'review', runtime, {for:'gather'})` — a bare 3-arg resolve of one refuses; a model-shaped slot is unaffected by purpose.
...
One dispatch on the **review** slot, two mandates, two finding vocabularies, one report. Verify, do
...
§3 "Review wave"). Both finding vocabularies and the one-blocker-pass cap
apply unchanged.
...
**The ratio ceiling is untouched by the shape rules.** The triad is the *shape* guide; the ratio ceiling is the *volume* brake, and two brakes on one axis would contradict. So: the ratio ceilings compute against the slice's **aggregate** source delta, not per cell; `new_suite_reason` governs a genuinely new suite file; a new test file on a `refactor`/`formatting` cell is refused outright. There is no numeric per-group cap.

**Per-cell proof scope, and the test-cell debt door.** Per-cell red-first/repro-first cells are not batched — but state the scope the way the proof-tier rules actually compute it, not "`bugfix` and `high-risk`", which is broader than the rules: `red-first` is `security`/`migration` in every lane plus `bugfix`/`behavior`/`api` at lane `high-risk`; `bugfix` below `high-risk` keeps repro-first on a `targeted-green` tier; and **the lane alone does not buy red-first** — at `high-risk`, `refactor`/`formatting` still resolve to `suite-green` and `test` to `targeted-green`.

The test-cell debt check has **no lane exemption**. It is keyed on the **feature**, not the slice, reads **no lane at all**, and its two refusal kinds do **not** share one predicate:

- *missing* — no `test` cell exists at all (a **dropped** one counts as none) **and** the feature has capped code-touching `behavior`/`api` cells; a cell whose recorded file list is empty or missing counts as code-touching.
- *not-green* — returns from the **offenders alone**, with no capped-behavior requirement whatsoever: a single `test` cell still open/claimed/blocked, capped with `verify_passed: false`, or capped with `trace.proof: "unrecorded"` refuses the close by itself.

Those three states are the whole offender predicate — **not** "capped green with recorded proof", which overstates it. A `test` cell capped on the `--feature-verify-pending` path carries `trace.feature_verify: "pending"` and never `proof: "unrecorded"`, so — provided no **failing** verify was recorded on it first, which still lands it in offenders — it clears *this* door and is held by the feature-verify debt check instead until one green feature verify answers for it. Neither kind is waivable — the test-cell door sits in the unwaivable feature-debt set, so no `gate_bypass` level, `total` included, lifts it.

So plan **one** trailing test cell even on a `high-risk` feature whose cells each carry per-cell red-first proof, and see it capped clear of the offender set — capped green with recorded output, or capped on the pending path whose bill the feature verify then pays. Creating the cell is not what satisfies the door; leaving it open, red, or proof-less is what keeps the door shut. Its coverage judgment will usually find the per-cell red-first proof already covers most of the story, which is the correct and cheap outcome. Do not work around the check, and do not silently drop the cell.
